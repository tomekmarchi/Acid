...
((global) => {
  ...
  $.range = range;
  const rangeRight = (start, stop, increment = 1) => {
    return rangeDown(stop, start, increment);
  };
  ...
  const drop = (array, amount, length) => {
  ...
  const findItem = (array, indexMatch, propertyName = 'id') => {
    const result = findArray(array, (element, index) => {
  ...
  const findIndex = (array, indexMatch, propertyName = 'id') => {
    const result = findIndexArray(array, (element, index) => {
      return findIndexCache(element, index, array, indexMatch, propertyName);
    });
    return (result === -1) ? false : result;
  };
  ...
  const partition = (array, funct) => {
    const temp = [];
    return [
      filterArray(array, (item, index) => {
        return funct(item) ? item : pushArray(temp, item) && undefined;
      }),
      temp,
    ];
  };
  ...
  const zip = function() {
    const args = arguments;
    return mapArray(args[0], (arraySet) => {
